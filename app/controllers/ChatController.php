...
class ChatController extends BaseController {
    ...
    public function createCuser() {
        $admin = CreateChatUserModel:: where('cname',Input:: get('chat_uname'))
                                       ->get(array('cname'));

        if(count($admin) > 0 and $admin[0]['cname'] == 'admin') {
            Session::put('owner', 'admin');
            CreateChatUserModel:: where('cname',Input:: get('chat_uname'))
            ...
            if(count($admin) > 0) {
                   ...
                   echo json_encode("Admin Offline.. Leave a message ....");
                }
            }
        }
   ...
   public function deleteChat() {
        $id = Input:: get('chat_id');
        // Need to get the status of admin to 0, if admin goes offline
        //CreateChatUserModel:: where('cname',Input:: get('chat_uname'))
        //                        ->update(array('status' => 0));
        $chat_user_deleted = CreateChatUserModel:: deleteChatUser($id);
        $chat_history_deleted = ChatSendReceiveModel:: deleteChatHistory($id);
        if ($chat_user_deleted and $chat_history_deleted)
            echo json_encode("1");
    }
